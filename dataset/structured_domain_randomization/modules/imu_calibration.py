...
def cal(C_imu: np.array, std_dev=0.1):
    ...
    Returns:
    - C_imu + noise: 2D numpy array, shape (N, 6), representing the input IMU data with added calibration-error-like noise.

    """
    noise = np.random.normal(0, std_dev, C_imu.shape)
    return C_imu + noise
